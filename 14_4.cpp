...
public: 
	void attach(Depositor *dps) { depositors.push_back(dps);}
...
public: 
	...
	void update(int money) { 
		deposit += money; std::cout << " Depositor_A's balance is "<< deposit << '\n'; 
	} 
}; 
...
public: 
	...
	void update(int money) { 
		deposit += money; std::cout << " Depositor_B's balance is " << deposit << '\n'; 
	} 
}; 
int main() { 
	Bank subj; 
