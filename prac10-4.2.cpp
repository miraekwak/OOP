...
public: 
	Intern(std::string major, std::string name, int age) 
		: majorName(major), Employee(name, age) {} void showInfo() 
	{ 
...
{ 
	int salary; 
public: 
	Janitor(int salary, std::string name, int age) 
		: salary(salary), Employee(name, age) {}
	void showInfo() { 
...
int main() { 
	// upcasting을 통해 각각 list를 만들필요없이 employeelist를 만들어 해결
